...
from sonar_analyzer.processing.filters import band_pass, high_pass, low_pass, notch
...
def apply_step(values: Samples, step: ProcessingStep) -> Samples:
    ...
    if step.kind is StepKind.LOW_PASS:
        return low_pass(
            data,
    ...
    if step.kind is StepKind.HIGH_PASS:
        return high_pass(
            data,
            float(params["sample_rate_hz"]),  # type: ignore[arg-type]
            float(params["cutoff_hz"]),  # type: ignore[arg-type]
    ...
    if step.kind is StepKind.NOTCH:
        return notch(
            data,
            float(params["sample_rate_hz"]),  # type: ignore[arg-type]
            float(params["center_hz"]),  # type: ignore[arg-type]
            float(params["q"]),  # type: ignore[arg-type]
            int(params["order"]),  # type: ignore[arg-type]
        )
