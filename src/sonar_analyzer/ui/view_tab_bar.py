...
Bu iş yalnız **yerleşimdir**. Yalnız `Time Series` bu fazda çalışıyor; kalan
yedi sekme plan Bölüm 22.4'e göre henüz desteklenmiyor ve **açıkça pasif**
gösteriliyor — gizlenmiyor, tıklanamıyor, nedeni ipucunda yazıyor:

* `Spectrum`, `Spectrogram` → Faz 4 (`F4-045` ve civarı).
* `Transmission` → Faz 3 (`F3-048`).
* `BIT / Status` → Faz 3.
...
#: Bu fazda calisan tek sekme.
ENABLED_TABS: frozenset[str] = frozenset({"Time Series"})
